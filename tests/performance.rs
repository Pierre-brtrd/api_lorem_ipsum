...
async fn test_concurrent_requests() {
    ...
    assert!(
        ...
        "Too slow for concurrent requests: {duration:?}"
    );
}
...
async fn test_large_text_generation() {
    ...
    assert!(
        ...
        "Should generate at least 1MB of text"
    );
    assert!(
        duration < Duration::from_millis(500),
        "Large text generation too slow: {duration:?}"
    );
}
...
async fn test_memory_efficiency() {
    ...
    assert!(
        ...
        "Memory leak detected: {memory_diff} bytes"
    );
}
